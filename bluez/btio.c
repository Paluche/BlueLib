...
static void accept_add(GIOChannel *io, BtIOConnect connect,
                       gpointer user_data, GDestroyNotify destroy)
{
...
static gboolean set_sec_level(int sock, BtIOType type, int level,
                              GError **err)
{
...
static gboolean get_peers(int sock, struct sockaddr *src,
                          struct sockaddr *dst, socklen_t len, GError **err)
{
    socklen_t olen;
