...
static int check_errors(int code)
{
  ...
  if (code != BL_NO_ERROR) {
    ...
    switch(code) {
      ...
      case BL_NOT_INIT_ERROR:
        while(bl_init(NULL, NULL, NULL, 0, TEST_SEC_LEVEL)) {
          printf("error count: %d\n", error_cnt);
          if (error_cnt < RETRY_MAX)
            error_cnt++;
          else
...
{
  printf("get_ble_tree  Copyright (C) 2013 Netatmo\nThis program comes with ABSOLUTELY NO WARR" \
      "ANTY\nThis is free software, and you are welcome to redistribute it\nunder certain condi" \
      "tions; See the GNU General Public License\nfor more details.\n\n");
  if(argc != 3) {
    usage();
    return 0;
  }
  ...
  do {
    bl_value = bl_read_char(GATT_CHARAC_DEVICE_NAME_STR, NULL, &gerr);
  ...
  do {
    bl_primary_list  = bl_get_all_primary(NULL, &gerr);
  } while (check_gerrors(gerr));
  ...
  if (bl_primary_list) {
    for (GSList *lp = bl_primary_list; lp; lp = lp->next) {
      ...
      do {
        bl_included_list = bl_get_included(bl_primary, &gerr);
      } while (check_gerrors(gerr));
      ...
      do {
        bl_char_list = bl_get_all_char_in_primary(bl_primary, &gerr);
      } while (check_gerrors(gerr));
      if (bl_char_list) {
        for (GSList *lc = bl_char_list; lc; lc = lc->next) {
          ...
          // Get all descriptors of the characteristic
          {
            ...
            do {
              bl_desc_list = bl_get_all_desc_by_char(bl_char,
                  next_bl_char, bl_primary, &gerr);
            } while (check_gerrors(gerr));
          }
          if (bl_desc_list)
